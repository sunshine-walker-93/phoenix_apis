...
for pb_source_path in ${pb_source_files}; do
  ...
  protoc -I="${wrapper_protoc_include:?is undefined}"  -I="${pb_source_dir}" --go_out=paths=source_relative:"${pb_target_dir}" --micro_out=paths=source_relative:"${pb_target_dir}" "${pb_source_rp}" &&
